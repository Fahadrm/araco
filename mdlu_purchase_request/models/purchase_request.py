...
class PurchaseRequest(models.Model):
    ...
    @api.depends('line_ids.product_qty', 'line_ids.price_unit', 'line_ids.sale_price')
    def _amount_all(self):
        """
        Compute the total amounts of the Purchase Request.
        """
        for order in self:
            untaxed_cost = cost_tax = taxed_cost = 0.0
            untaxed_selling = selling_tax = taxed_selling = 0.0
            untaxed_markup = taxed_markup = 0.0
            for line in order.line_ids:
                untaxed_cost += line.product_qty * line.price_unit
                untaxed_selling += line.product_qty * line.sale_price
            cost_tax = (untaxed_cost * 15)/100
            taxed_cost = untaxed_cost + cost_tax
            selling_tax = (untaxed_selling*15)/100
            taxed_selling = untaxed_selling + selling_tax
            untaxed_markup = ((untaxed_selling-untaxed_cost)/untaxed_cost)*100 if untaxed_cost !=0 else 0
            taxed_markup = ((taxed_selling - taxed_cost)/taxed_cost)*100 if taxed_cost != 0 else 0

            order.update({
                'untaxed_cost': untaxed_cost,
                'cost_tax': cost_tax,
                'taxed_cost': taxed_cost,
                'untaxed_selling': untaxed_selling,
                'selling_tax': selling_tax,
                'taxed_selling': taxed_selling,
                'untaxed_markup': untaxed_markup,
                'taxed_markup': taxed_markup,
            })
    ...
    user_id = fields.Many2one('res.users', 'Requested by', track_visibility='onchange',
                              domain=[('partner_id.purchase_request_allowed', '=', True)],
    ...
    line_ids = fields.One2many('purchase.request.line', 'request_id', 'Products to Purchase', readonly=False, copy=True,
                               track_visibility='onchange')
    state = fields.Selection(selection=_STATES, string='Status', index=True, track_visibility=True, required=True,
                             copy=False, default='draft')
    is_editable = fields.Boolean(string="Is editable", compute="_compute_is_editable", readonly=True)
    picking_type_id = fields.Many2one('stock.picking.type', 'Picking Type', required=True,
                                      default=_default_picking_type)
    po_ids = fields.Many2many('purchase.order', 'purchase_request_purchase_order_rel', 'request_id', 'po_id',
                              string="Purchase Orders")
    po_count = fields.Integer(compute='_count_pos')
    untaxed_cost = fields.Float(string='Untaxed Cost', store=True, readonly=True, compute='_amount_all')
    cost_tax = fields.Float(string='VAT (15%)', store=True, readonly=True, compute='_amount_all')
    taxed_cost = fields.Float(string='Total Cost', store=True, readonly=True, compute='_amount_all')
    untaxed_selling = fields.Float(string='Untaxed Selling', store=True, readonly=True, compute='_amount_all')
    selling_tax = fields.Float(string='VAT (15%)', store=True, readonly=True, compute='_amount_all')
    taxed_selling = fields.Float(string='Total Selling', store=True, readonly=True, compute='_amount_all')
    untaxed_markup = fields.Float(string='Markup Before Tax', store=True, readonly=True, compute='_amount_all')
    taxed_markup = fields.Float(string='Markup After Tax', store=True, readonly=True, compute='_amount_all')
    ...
    def write(self, vals):
        ...
        for request in self:
            ...
            if vals.get('assigned_to'):
                follower_ids.append(
                    self.env['res.users'].search([('id', '=', vals['assigned_to'])], limit=1).partner_id.id)
            if request.line_ids:
                follower_ids.extend(request.line_ids.mapped('second_approver.partner_id.id'))
    ...
    def button_approved(self):
        ...
        for rec in self:
            for line in rec.line_ids:
                ...
                if line.state == 'approved':
                    ...
                    po = self.env['purchase.order'].search(
                        [('state', '=', 'draft'), ('partner_id', '=', line.vendor_id.id), ('partner_ref', '=', False)])
                    ...
                    new_po_line = self.env['purchase.order.line'].search(
                        [('order_id', '=', po.id), ('product_id', '=', line.product_id.id), ('name', '=', rec.name)])
            ...
            rec_lines = self.env['purchase.request.line'].search(
                [('request_id', '=', rec.id), ('state', 'not in', ['ordered', 'cancelled', 'rejected'])])
            if not rec_lines:
                rec.state = 'approved'
        return pos, po_lines
...
class PurchaseRequestLine(models.Model):
    ...
    def _calculate_default_price(self):
        for line in self:
            ...
            if line.product_id.variant_seller_ids:
                seller_product_price = self.env['product.supplierinfo'].search(
                    [('product_tmpl_id', '=', product.product_tmpl_id.id), ('name', '=', vendor.id),
                     ('product_id', '=', product.id)], limit=1).price
                seller_price = self.env['product.supplierinfo'].search(
                    [('product_tmpl_id', '=', product.product_tmpl_id.id), ('name', '=', vendor.id),
                     ('product_id', '=', False)], limit=1).price
    ...
    @api.depends('product_qty', 'sale_price', 'price_unit')
    def _compute_markup(self):
        """
        Compute the markup of the PR line.
        """
        for line in self:
            sale = line.product_qty*line.sale_price
            cost = line.product_qty*line.price_unit
            price = ((sale-cost)/cost)*100 if cost != 0 else 0
    ...
    product_uom_id = fields.Many2one('uom.uom', string='Product Unit of Measure', default=_default_product_uom,
    ...
    date_required = fields.Date(string='Item Request Date', required=True, track_visibility='onchange',
    ...
    additional_approval = fields.Boolean(string='Needs Additional Approval', compute="_compute_additional_approval",
                                         readonly=True)
    reason = fields.Char(string="Reason Requested")
    price_unit = fields.Float(string='Unit Price', digits='PR Unit Price', default=_calculate_default_price)
    sale_price = fields.Float(string='Sale Price', digits='Product Price')
    standard_price = fields.Float('Cost', digits='Product Price')
    qty_available = fields.Float('Quantity On Hand', digits='Product Unit of Measure')
    markup = fields.Float(compute='_compute_markup', string='%', readonly=True, store=True)
