...
class PurchaseOrder(models.Model):
    ...
    def create(self, vals):
        ...
        if 'purchase_creation' in vals:
            vals['purchase_creation'] = self.env.uid if 'purchase_creation' in vals else ""
        if 'purchase_creation_date' in vals:
            vals['purchase_creation_date'] = datetime.now().strftime(
                    "%Y-%m-%d %H:%M:%S") if 'purchase_creation_date' in vals else ""
                # vals['purchase_creation']  = fields.Datetime.context_timestamp(self, fields.Datetime.to_datetime(vals['date_order']))
