    ...
    def security_for_validate(self):
        for order in self:
            ...
            if order.picking_type_code == 'internal':
                # if order.state in ['send_to_logistic_approval','send_to_manager_approval','ready_to_transfer','stock_in_transist']:

                if order.state in ['draft','send_to_logistic_approval','send_to_manager_approval','ready_to_transfer','stock_in_transist','done']:
